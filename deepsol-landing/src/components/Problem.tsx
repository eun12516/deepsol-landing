...
export default function Problem() {
  return (
    <section className="w-full flex flex-col items-center justify-center py-20 px-4 bg-gradient-to-br from-blue-50/70 via-white to-blue-100/60">
      <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-gray-900 text-center mb-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 w-full max-w-4xl mx-auto">
        {problems.map((problem, idx) => (
          <motion.article
            ...
            className="flex flex-col items-start bg-white/60 border border-white/40 rounded-2xl p-8 gap-4 h-full justify-between shadow-xl backdrop-blur-md transition-transform duration-300 hover:scale-105 hover:shadow-blue-200/70 hover:ring-2 hover:ring-blue-200/60"
          >
            <problem.icon className="w-10 h-10 text-[#3182f6] mb-2" aria-hidden="true" />
            <h3 className="text-xl font-bold text-gray-900 mb-1">{problem.title}</h3>
            <p className="text-gray-600 text-base mb-4">{problem.desc}</p>
            <div className="mt-auto text-[#3182f6] font-semibold text-base bg-[#f0f6fe] rounded-lg px-4 py-2">
              {problem.highlight}
            </div>
          </motion.article>
        ))}
